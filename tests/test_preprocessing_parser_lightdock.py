...
def parser():
    ...
    return ParserLightDock(working_dir=base_dir, score_filename='gso_100.out')
...
class TestParserLightDock:
    """
    It wraps all tests that involve the ParserLightDock class
    """
    def test_initialization(self, tmp_path):
        ...
        base_dir, *c = make_directory_structure(tmp_path, program, sc_filename,
                                                is_lightdock=True)
        p = ParserLightDock(working_dir=base_dir, score_filename=sc_filename)
        assert isinstance(p, ParserLightDock)
    ...
    def test_validate_read(self, parser):
        ...
        desired_col_names = ['swarm', 'RecID', 'LigID', 'Luciferin',
    ...
    def test_invalid_sc_file(self, tmp_path):
        ...
        with pytest.raises(Exception):
            ParserLightDock(working_dir=tmp_path, score_filename='dummy')
